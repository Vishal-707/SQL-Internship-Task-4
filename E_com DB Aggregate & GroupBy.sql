...
--  An aggregate function is a function that performs a calculation on a set of values, and returns a single value. ex.SUM MAX MIN AVG etc.

--  SUM of all Order Amount 
SELECT SUM(Amount) AS Total_Amount FROM Orders; -- SUM fun is used to Sum of all amount from orders table 

-- COUNT customers 
SELECT COUNT(Customer_id) AS Number_Of_Custmer FROM Customers; -- To check Total customers 

-- Find max price in product 
SELECT MAX(price) AS Total_Price FROM Products;

-- Aggregate functions are often used with the GROUP BY clause of the SELECT statement. The GROUP BY clause splits the result-set into groups 
-- of values and the aggregate function can be used to return a single value for each group. Following Example 

 -- -- GROUP BY -- -- 
 
--  Revenue and Orders per Payment Type.
SELECT Payment_Type, COUNT(Order_Id) AS NO_OF_ORDER , SUM(Amount) AS Total_Revenue FROM Orders
GROUP BY Payment_Type ORDER BY Total_Revenue DESC;  -- Groups the orders by their Payment_Type and calculates the total revenue and the number of orders for each type.

-- What is the total number of Category 
SELECT Category , COUNT(category) AS Count FROM Products GROUP BY Category ; 

-- What is the total amount of sales and the average amount of a single order for each distinct payment type (UPI, CARD, CASH)?
SELECT Payment_Type, SUM(Amount) AS Total_Revenue , AVG(Amount) AS Average_Order_Amount
FROM Orders GROUP BY Payment_Type ORDER BY Total_Revenue DESC; 

						--   -- Filter Groups using HAVING --  --
					
-- Stock Quantity per Category 
SELECT Category, SUM(Stock_quantity) AS Total_Stock FROM Products
GROUP BY Category HAVING Total_Stock > 1000 ORDER BY Total_Stock DESC;
			-- The HAVING clause then filters this result to show only categories with a total stock of Greater than 1000 units.
		
-- Which payment types have been used for a total revenue exceeding 200,000? List the payment type and its total revenue.
SELECT Payment_Type, SUM(Amount) AS Total_Revenue FROM Orders
GROUP BY Payment_Type HAVING Total_Revenue > 200000 ORDER BY Total_Revenue DESC; 

				-- Combibned Aggregate Functions, GROUP BY, and HAVING --

-- This query combines all three concepts to give you meaningful insights:
 --  Category Performance Analysis .
 SELECT P.Category, COUNT(O.Order_Id) AS Total_Orders, SUM(O.Amount) AS Total_Sales, AVG(O.Amount) AS Average_Order_Amount, -- Aggregate fun 
 MAX(O.Amount) AS Max_Order_Amount FROM Orders O JOIN Products P ON O.Product_Id = P.Product_Id  -- join use
GROUP BY P.Category HAVING Total_Sales > 50000 ORDER BY Total_Sales DESC;   -- Group by and Having Fillter 
